...
export const bookingApi = createApi({
  ...
  endpoints: (builder) => ({
    // 获取所有 booking，带 cabin 和 guest 信息
    ...
    getBookingsAfterDate: builder.query({
      query: (date) =>
        `/bookings?populate=cabinID&populate=guestID&filters[createdAt][$gte]=${date}`,
      providesTags: ['Bookings'],
    }),
    ...
    getStaysAfterDate: builder.query({
      query: (date) =>
        `/bookings?populate=guestID&filters[startDate][$gte]=${date}`,
      providesTags: ['Bookings'],
    }),
...
getStaysTodayActivity: builder.query({
  query: () => {
    ...
    const query = `/bookings?populate=cabinID&populate=guestID` +
  ...
  providesTags: ['Bookings'],
    ...
    deleteBooking: builder.mutation({
      ...
      invalidatesTags: ['Bookings'], // 删除后使缓存失效,这样会直接更新新的数据
    }),
  }),
});
